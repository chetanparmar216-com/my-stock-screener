...
st.title("📊 NSE Ultimate F&O Screener (With Refresh Controls)")

# Sidebar Settings
st.sidebar.header("⚙️ Scanner Settings")

# 1. Auto Refresh ON/OFF Feature
auto_refresh = st.sidebar.checkbox("🔄 Enable Auto-Refresh", value=True)

# 2. Refresh Interval Slider
refresh_sec = st.sidebar.slider("Auto-Refresh Rate (Seconds):", min_value=10, max_value=120, value=30, disabled=not auto_refresh)

# 3. Manual Refresh Button (Useful when Auto-Refresh is OFF)
if st.sidebar.button("🔄 Refresh Data Manually"):
    st.rerun()

st.sidebar.caption("Tip: Turn off auto-refresh if you want to study data without lists jumping around.")
