...
st.title("📊 NSE Ultimate F&O Screener (Live Auto-Update)")

# Sidebar Settings
st.sidebar.header("⚙️ Scanner Settings")
refresh_sec = st.sidebar.slider("Auto-Refresh Rate (Seconds):", min_value=10, max_value=120, value=30)
st.sidebar.caption("Data will auto-refresh automatically in background.")
...
def fetch_stock_data(ticker):
    try:
        ...
        inst_action = "Neutral"
        if curr['Close'] > prev['Close'] and vol_ratio >= 1.5 and curr['OBV'] > curr['OBV_EMA']:
            inst_action = "🟢 Heavy Buying"
        elif curr['Close'] < prev['Close'] and vol_ratio >= 1.5 and curr['OBV'] < curr['OBV_EMA']:
...
# Auto Refreshing Fragment Block
@st.fragment(run_every=refresh_sec)
def render_screener_dashboard():
    with st.spinner("Market F&O Universe auto-updating..."):
    ...
    st.caption(f"⏱️ Last auto-updated: {time.strftime('%H:%M:%S IST')} | Total Stocks Scanned: {len(df)}")

    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "⚡ Intraday", "🌙 BTST", "📈 Swing", "🏛️ Institutional Activity", "📋 All F&O Universe"
    ])
    ...
    with tab1:
        ...
        if not intraday_df.empty:
            ...
            st.info("Filhaal koi stock Previous Day High ke upar breakout nahi bana raha hai.")
    ...
    with tab2:
        ...
        if not btst_df.empty:
            ...
            st.info("Filhaal koi stock BTST criteria match nahi kar raha hai.")
    ...
    with tab4:
        st.subheader("🏛️ Institutional Accumulation / Distribution (Big Money)")
        ...
        if not inst_df.empty:
            ...
            st.info("Filhaal kisi F&O stock me abnormal institutional volume surge trigger nahi hua hai.")

    with tab5:
        st.subheader("📋 All F&O Stocks Complete List")
        st.dataframe(df[['Symbol', 'LTP', 'Prev_High', 'Institutional Flow', 'Vol_Ratio', 'RSI', 'EMA_20', 'EMA_50', 'EMA_200']], use_container_width=True)
